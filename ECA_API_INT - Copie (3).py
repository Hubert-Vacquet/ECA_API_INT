...
# Liste simulée des PDL activés pour test
...
@app.route('/housing/<housing_id>', methods=['DELETE'])
def delete_housing(housing_id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()

        cursor.execute("DELETE FROM HOUSINGS WHERE ID = ?", (housing_id,))
        conn.commit()
...
@app.route('/api/validate_enedis', methods=['POST'])
def validate_enedis():
    ...
    if pdl in PDL_ACTIVÉS:
        ...
        # Simule une "vérification" lente ou réelle ici si besoin
